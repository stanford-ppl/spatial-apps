...
psim_generic("psim_v2_s4"        , ["psim_p2p_ideal"] , "--net=static --vlink=2 --slink=4 --fifo-depth=4 --vfifo=4 --link-prop=db --flit-width=512" )
...
def runbest():
    opts.toclear = reduce(lambda a,b: "{},{}".format(a,b), [ p for p in passes() if
        p.startswith("psim") and "D" in p and 'q' in p])
    # opts.toclear = reduce(lambda a,b: "{},{}".format(a,b), [ p for p in passes() if
        # p=='link_count'])
    if opts.run:
        opts.git=False
