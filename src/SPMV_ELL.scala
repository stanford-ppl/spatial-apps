 ...
 */
  val appName = s"$this".split("@")(0).replace("$","")
  ...
  def generateData = {
    ...
    toFile(denseMat, sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_mat.csv")
    toFile(sparseValues, sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_values.csv")
    toFile(sparseCols, sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_cols.csv")
    toFile(denseVec, sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_vec.csv")
    toFile(denseGold, sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_gold.csv")
  }
  ...
  def main() = {
    ...
    generateData
    val raw_values = loadCSV2D[T](sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_values.csv")
    ...
    val data_gold = loadCSV1D[T](sys.env("SPATIAL_HOME") + s"/gen/$appName/ell_gold.csv", "\n")
