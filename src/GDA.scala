...
object GDA extends SpatialApp { // Regression (Dense) // Args: 64
  ...
  def gda[T: Type : Num](xCPU: Array[T], yCPU: Array[Int], mu0CPU: Array[T], mu1CPU: Array[T]) = {
    ...
    Accel {
      ...
      Parallel {
        mu0Tile load mu0(0 :: C par loadPar) // Load mu0
        mu1Tile load mu1(0 :: C par loadPar) // Load mu1
      }
      ...
      MemReduce(sigmaOut)(R by rTileSize par op){ r =>
        ...
        val blk = Reg[Int]
        Parallel {
          gdaYtile load y(r :: r + rTileSize par loadPar)
          gdaXtile load x(r :: r + rTileSize, 0 :: C par loadPar) // Load tile of x
          Pipe {
        ...
        MemReduce(sigmaBlk)(blk par mp) { rr =>
      ...
      sigma(0 :: C, 0 :: C par storePar) store sigmaOut
    }
