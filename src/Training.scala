...
// object HALP extends SpatialApp {  // Test Args: 25 30 256 0.0001 0.0009 10
...
//   @struct case class halp_number(original_bits: Int, dec_interpretation: Int)
//   type HP = FixPt[TRUE, _32, _32]
...
//     val len_epoch = args(1).to[Int] // Epoch Length
//     val points = args(2).to[Int] // Total Points
//     val alpha1 = args(3).to[Int] // Step size
//     val alpha2 = args(4).to[Int] // Step size
//     val D = 16
//     val ofsM = 2 // initial model points are to be interpretted ofsM dp from lsb
//     val ofsX = 2 // initial X and Y points are to be interpretted ofsX dp from lsb
//     val bump_epoch = args(5).to[Int]

//     val noise_num = 2
//     val noise_denom = 10
//     // Generate some test data
//     val sX = (0::points, 0::D){(i,j) => (random[Int](3) + 1) << ofsX}
//     val W_gold = Array.tabulate(D) { i => (random[Int](3) / 2) << offsM}
//     val sY = Array.tabulate(points) { i => ((random[HP](noise_num.to[HP]) / noise_denom.to[HP] - noise_num.to[HP]/2) << ofsX).to[Int] + Array.tabulate(D){j => (W_gold(j) * sX(i,j)) >> (ofsX+ofsM)}.reduce{_+_} }
...
//       val w_k = SRAM[Int](D)
//       val g_k = SRAM[Int](D)
//       val y_cache = SRAM[Int](tileSize)
//       val y_cache_base = Reg[Int](0)

//       Pipe(D by 1 par P2) { i => w_k(i) = 0.to[Int] }
//       y_cache load y(0::tileSize par loadPar)

//       val center_ofs = Reg[Int](0) // Number of bits << to place decimal
