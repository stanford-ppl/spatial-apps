...
trait CellsPar extends SpatialApp {
  ...
  def tanh_synth[T:Type:Num](p: T) = {
    ...
    val cond1 = (absp > (2.5).to[tanhT])
    val body1 = 1.to[tanhT]

    val cond2 = (((0.5).to[tanhT] < absp) && (absp <= (2.5).to[tanhT]))
    // bug: if replace div4 with the shifted result directly, spatial would infer the type of absp >> 2 as FixPt[TRUE, 0, 3]
    val div4 = absp >> 2
    val div4Offset = div4 + (0.375).to[tanhT]
    val body2 = mux(cond2, div4Offset, absp)

    val absre = mux(cond1, body1, body2)
  ...
  case class CellParam[T:Type:Num](paramPath: java.lang.String, cellName: java.lang.String,
    ...
    val kernelColSize = hidden_size * 4
    
    println(s"kernel size = $kernelReduceSize by $kernelColSize")
    println(s"bias size = $batch_size by $linear_output_size")
    println(s"")
  ...
  @virtualize
    ...
    def basicGate(x: SRAM2[T], h: SRAM2[T], gateParams: GateParams[T], forgetBias: Int, activation: T => T) {
      ...
      // TODO: how can I rewrite this part so that it can par?
        ...
        Foreach (batch_size by 1 par midPar) { i =>
          Foreach (dm by 1 par innerPar) { jj =>
            val prod = Reduce(Reg[T]) (dp by 1) { kk =>
              ...
              //}
              val tk = tileKernel(kk, jj)
              val tempA = x(i, reduce_size_offset) * tk
              val tempB = h(i, reduce_size_offset - feature_size) * tk
            ...
            //}
            targetSRAM(i, colOffset) = mux(k >= kernelReduceSize - dp, activation(ele + forgetBias.to[T]), ele)
          }
        }
      }
    }
  ...
  case class BasicLSTMCell[T <: MetaAny[T] :Type:Num](cellParam: CellParam[T], override val x: SRAM2[T]) extends RNNCell[T](cellParam, x) {
    ...
    def forward(isLastIter: Boolean = false) {
      ...
      // TODO: seems that this can be a reduce that I could expres?
  ...
  case class ResidualBasicLSTMCell[T <: MetaAny[T] :Type:Num ](cellParam: CellParam[T], override val x: SRAM2[T]) extends RNNCell[T](cellParam, x) {
    ...
    def forward(isLastIter: Boolean = false) {
      ...
      Foreach (batch_size by 1, hidden_size by 1 par elePar) { (i,j) =>
        ...
        //}
        val prod = tanh_synth[T](new_c) * sigO(i,j) 
        h (i,j) = mux(isLastIter, prod + x(i,j), prod)
        c(i,j) = new_c
      }
    }
  }
        ...
        Foreach (batch_size by 1 par midPar) { i =>
          Foreach (dm by 1 par innerPar) { jj =>
            val prod = Reduce(Reg[T]) (dp by 1) { kk =>
              ...
              val tk = tileKernel(kk, jj)
              mux(
                reduce_size_offset < feature_size,
                x0(i, reduce_size_offset) * tk,
                mux(
                  feature_size <= reduce_size_offset && reduce_size_offset < xxFeatureSize,
                  x1(i, reduce_size_offset - feature_size) * tk,
                  h(i, reduce_size_offset - xxFeatureSize) * tk 
                )
              )
            } {_+_}
            ...
            //} else {
              //targetSRAM(i, colOffset) = ele
            //}
            targetSRAM(i, colOffset) = mux(
              k >= kernelReduceSize - dp,
              activation(ele + forgetBias.to[T]),
              ele
            )
          }
        }
